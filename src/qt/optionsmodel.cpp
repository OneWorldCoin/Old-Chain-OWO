...
void OptionsModel::Init()
{
    ...
    if (!settings.contains("nAnonymizeOneWorldAmount"))
        settings.setValue("nAnonymizeOneWorldAmount", 1000);

    nAnonymizeOneWorldAmount = settings.value("nAnonymizeOneWorldAmount").toLongLong();
...
// Wallet
    ...
    if (settings.contains("nAnonymizeOneWorldAmount"))
        SoftSetArg("-anonymizeoneworldamount", settings.value("nAnonymizeOneWorldAmount").toString().toStdString());
...
void OptionsModel::Reset()
{
    ...
    settings.clear();
    resetSettings = true; // Needed in oneworld.cpp during shotdown to also remove the window positions
...
QVariant OptionsModel::data(const QModelIndex& index, int role) const
{
    if (role == Qt::EditRole) {
        ...
        case AnonymizeOneWorldAmount:
            return QVariant(nAnonymizeOneWorldAmount);
        case Listen:
...
bool OptionsModel::setData(const QModelIndex& index, const QVariant& value, int role)
{
    ...
    if (role == Qt::EditRole) {
        ...
        case AnonymizeOneWorldAmount:
            nAnonymizeOneWorldAmount = value.toInt();
            settings.setValue("nAnonymizeOneWorldAmount", nAnonymizeOneWorldAmount);
            emit anonymizeOneWorldAmountChanged(nAnonymizeOneWorldAmount);
            break;
        case CoinControlFeatures:
