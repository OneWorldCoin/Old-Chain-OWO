...
{
    // return if URI is not valid or is no OneWorld: URI
...
{
    // Convert oneworld:// to oneworld:
    //
    //    Cannot handle this later, because oneworld:// will cause Qt to see the part after // as host,
...
void openConfigfile()
{
    ...
    /* Open oneworld.conf with the associated application */
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "OneWorld.lnk";
}

bool GetStartOnSystemStartup()
{
    // check for OneWorld.lnk
...
{
    return GetAutostartDir() / "oneworld.desktop";
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    if (!fAutoStart)
        ...
        if (!optionFile.good())
            return false;
        // Write a oneworld.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        optionFile << "Name=OneWorld\n";
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the oneworld app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if (fAutoStart && !foundItem) {
        // add oneworld app to startup item list
