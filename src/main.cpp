...
#if defined(NDEBUG)
#error "OneWorld cannot be compiled without assertions."
#endif
...

    {
        ...
        if(tx.IsZerocoinSpend()){
            ...
            if (IsTransactionInChain(tx.GetHash(), nHeightTx))
                return state.Invalid(error("AcceptToMemoryPool : zOWO spend tx %s already in block %d", tx.GetHash().GetHex(), nHeightTx),
            ...
            for (const CTxIn& txIn : tx.vin) {
                ...
                if (IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTx))
                    return state.Invalid(error("%s : zOWO spend with serial %s is already in block %d\n",
                ...
                if (!spend.HasValidSerial(Params().Zerocoin_Params()))
                    return state.Invalid(error("%s : zOWO spend with serial %s from tx %s is not in valid range\n",
...
int64_t GetBlockValue(int nHeight)
{
    int64_t nSubsidy = 0;
      ...
      if (nHeight == 0) {
             nSubsidy = 5905339 * COIN;
      } else if (nHeight == 1) {
             nSubsidy = 20000000 * COIN;
      } else if (nHeight <= 6002 && nHeight > 1) {
             nSubsidy = 2 * COIN;
      } else if (nHeight <= 10000 && nHeight > 6002) {
             nSubsidy = 100 * COIN;
      } else if (nHeight <= 30001 && nHeight > 10000) {
             nSubsidy = 75 * COIN;
      } else if (nHeight <= 50002 && nHeight > 30001) {
             nSubsidy = 140 * COIN;
      } else if (nHeight <= 80003 && nHeight > 50002) {
             nSubsidy = 210 * COIN;
      } else if (nHeight <= 110004 && nHeight > 80003) {
             nSubsidy = 280 * COIN;
      } else if (nHeight <= 150005 && nHeight > 110004) {
             nSubsidy = 350 * COIN;
      } else if (nHeight <= 200006 && nHeight > 150005) {
             nSubsidy = 410 * COIN;
      } else if (nHeight <= 266007 && nHeight > 200006) {
             nSubsidy = 325 * COIN;
      } else if (nHeight <= 350008 && nHeight > 266007) {
             nSubsidy = 300 * COIN;
      } else if (nHeight <= 435009 && nHeight > 350008) {
             nSubsidy = 275 * COIN;
      } else if (nHeight <= 525610 && nHeight > 435009) {
             nSubsidy = 250 * COIN;
      } else if (nHeight <= 657011 && nHeight > 525610) {
             nSubsidy = 240 * COIN;
      } else if (nHeight <= 788412 && nHeight > 657011) {
             nSubsidy = 235 * COIN;
      } else if (nHeight <= 1054413 && nHeight > 788412) {
             nSubsidy = 190 * COIN;
      } else if (nHeight <= 1320415 && nHeight > 1054413) {
             nSubsidy = 140 * COIN;
      } else if (nHeight <= 1586416 && nHeight > 1320415) {
             nSubsidy = 90 * COIN;
      } else if (nHeight <= 1626417 && nHeight > 1586416) {
             nSubsidy = 85 * COIN;
      } else if (nHeight <= 1666418 && nHeight > 1626417) {
             nSubsidy = 70 * COIN;
      } else if (nHeight <= 5256000 && nHeight > 1666418) {
             nSubsidy = 41 * COIN;
      } else {
          nSubsidy = 0.1 * COIN;
      }
...
int64_t GetMasternodePayment(int nHeight, int64_t blockValue, int nMasternodeCount)
{
	...
	if (nHeight <= 200) {
        ...
        } else if (nHeight <= 1586416 && nHeight > 10000 ) {
...
bool DisconnectBlock(CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool* pfClean)
{
    ...
    for (int i = block.vtx.size() - 1; i >= 0; i--) {
         ...
         * note we only undo zerocoin databasing in the following statement, value to and from OneWorld
...
{
    RenameThread("oneworld-scriptch");
    scriptcheckqueue.Thread();
}

void RecalculateZONEWORLDMinted()
{
...
void RecalculateZONEWORLDSpent()
{
    CBlockIndex* pindex = chainActive[Params().Zerocoin_AccumulatorStartHeight()];
    while (true) {
        if (pindex->nHeight % 1000 == 0)
        ...
        //Remove spends from zOWO supply
...
bool RecalculateONEWORLDSupply(int nHeightStart)
{
                ...
                if (zerocoinDB->ReadCoinSpend(spend.getCoinSerialNumber(), hashTxFromDB)) {
                    if(IsSerialInBlockchain(spend.getCoinSerialNumber(), nHeightTxSpend)) {
                        if(!fVerifyingBlocks || (fVerifyingBlocks && pindex->nHeight > nHeightTxSpend))
                            return state.DoS(100, error("%s : zOWO with serial %s is already in the block %d\n",
...
//    LogPrintf("XX69----------> ConnectBlock(): nValueOut: %s, nValueIn: %s, nFees: %s, nMint: %s zOWOSpent: %s\n",
...
void static UpdateTip(CBlockIndex* pindexNew)
{
    ...
    // If turned on AutoZeromint will automatically convert ONEWORLD to zOWO
...
bool CheckBlock(const CBlock& block, CValidationState& state, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig)
{
    ...
    for (const CTransaction& tx : block.vtx) {
        ...
        // double check that there are no double spent zOWO spends in this block
        if (tx.IsZerocoinSpend()) {
            for (const CTxIn txIn : tx.vin) {
                if (txIn.scriptSig.IsZerocoinSpend()) {
                    libzerocoin::CoinSpend spend = TxInToZerocoinSpend(txIn);
                    if (count(vBlockSerials.begin(), vBlockSerials.end(), spend.getCoinSerialNumber()))
                        return state.DoS(100, error("%s : Double spending of zOWO serial %s in block\n Block: %s",
...
bool ProcessNewBlock(CValidationState& state, CNode* pfrom, CBlock* pblock, CDiskBlockPos* dbp)
{
    ...
    if (nMints || nSpends)
        LogPrintf("%s : block contains %d zOWO mints and %d zOWO spends\n", __func__, nMints, nSpends);
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv, int64_t nTimeReceived)
{
    ...
    if (strCommand == "version") {
        ...
        // OneWorld: We use certain sporks during IBD, so check to see if they are
